...
def create_calculator_tool(logger: ToolLogger):
    """
    Create and return a tool for safely evaluating basic arithmetic expressions.
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A string containing a mathematical expression (e.g., "2 + 2 * (3 - 1)")

        Returns:
            The result of the evaluated expression as a string, or an error message if invalid.
        """
        
        try:
            if not expression.strip():
                raise ValueError("Error: Empty expression provided.")
            if not allowed_chars.fullmatch(expression):
                raise ValueError("Invalid characters in expression. Only numbers and +, -, *, /, (, ) are allowed.")
            if "**" in expression:
                raise ValueError("Exponentiation using ** is not allowed.")
